...
@router.post("/", response_model=CustomResponseSchema)
...
@router.get("/", response_model=CustomResponseSchema)
async def list_journal(journal_service=Depends(get_journal_service)):
    """
    Creates a new journal
    Returns:
        JSON : with all journal associated with the given user
    """
    data = await journal_service.list_journals()
